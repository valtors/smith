...
use smith_compose::{list_all_tools, run_compose_server};
use smith_config::{load, save};
use smith_index::{
    build_index, format_index_compact, format_stats, get_schema_json, ComposedToolRef,
};
    ...
    match cli.command {
        ...
        Commands::Compose => {
            ...
            if config.active_servers().is_empty() {
                eprintln!(
                    "no active servers. install one: smith install @modelcontextprotocol/filesystem"
                );
                return;
            }
            println!(
        ...
        Commands::Index { stats, json, get } => {
            ...
            if tools.is_empty() {
                eprintln!("no tools found from active servers");
                return;
            }
            let refs: Vec<ComposedToolRef> = tools
                .iter()
                .map(|t| ComposedToolRef {
            ...
            if let Some(tool_name) = get {
                match get_schema_json(&refs, &tool_name) {
                    Some(schema) => println!("{}", serde_json::to_string_pretty(&schema).unwrap_or_default()),
            ...
            print!("{}", format_index_compact(&index));
        }
    }
}
